...
/**
 * Created by nfs05 on 29/12/2017.
 */
...
public class QueryUtils {
    ...
    public static ArrayList<QuakeInfo> extractEarthQuakes(){
       ArrayList<QuakeInfo> mList = new ArrayList<>();
       try{
          JSONObject root = new JSONObject(SAMPLE_JSON_RESPONSE);
          // extract json array .
           JSONArray jsonArray = root.getJSONArray("features");
           // Here looping for jsonArray
           for (int i = 0 ; i < jsonArray.length() ; i++){
              // Create object of QuakeInfo class
               JSONObject index = jsonArray.getJSONObject(i);
               JSONObject properties = index.getJSONObject("properties");
               double mag = properties.getDouble("mag");
               String place  = properties.getString("place");
               long currentTime= properties.getLong("time");
               String url = properties.getString("url");
//               Date date = new Date(currentTime);
//               DateFormat toDay = new SimpleDateFormat("dd/MM/yyyy : hh:mm:ss");
//               String t = toDay.format(date);
               QuakeInfo quake = new QuakeInfo(mag,place,currentTime,url);
//               quake.setMagnitude(mag);
//               quake.setQuakeLocation(place);
//               quake.setQuakeData(time);
               mList.add(quake);
           }
       }catch (JSONException e){
           e.printStackTrace();
           Log.e("Quake","Problem parsing data,Can't fetch data");
